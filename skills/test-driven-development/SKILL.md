...
- Read-only diagnosis, log analysis, or configuration lookup
- Comment-only changes with no behavior change
...
## Define Verifiable Behavior First
...
If the current environment cannot run automated tests, state the blocker and
provide reproducible manual verification steps. Manual verification does not
replace the follow-up need for automated regression coverage.
...
| "This is just a hotfix" | Make the smallest safe fix, record why TDD was constrained, then add regression coverage in this or the next nearest slice. |
...
- [ ] If automation was blocked, blocker and manual verification steps are documented
