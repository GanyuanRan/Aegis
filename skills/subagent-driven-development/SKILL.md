...
digraph process {
    ...
    "Use aegis:finishing-a-development-branch" [shape=box style=filled fillcolor=lightgreen];
    ...
    "Dispatch final code reviewer subagent for entire implementation" -> "Use aegis:finishing-a-development-branch";
}
```

Before dispatching the first implementer for a multi-task plan, load aegis:long-task-continuation and create a checkpoint. Include the current checkpoint and resume constraints in every implementer prompt.
...
[Read plan file once: docs/aegis/plans/feature-plan.md]
...
You: "User level (~/.config/aegis/hooks/)"
