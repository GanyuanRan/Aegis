...
each prompt-debug invocation; role, content, type and nested fields remain part
of the comparison contract.

The outer benchmark `bwrap` boundary consumes the host user namespace, so a
Codex tool sandbox that requires a second `bwrap` namespace is incompatible
with this topology. The frozen host path must select a nested-compatible,
network-denying sandbox backend and prove it starts in both arms with a
zero-inference tool-sandbox probe before provider inference. A sandbox backend
failure during an attempt is infrastructure-invalid and must never be scored as
an agent outcome. Bypassing the Codex tool sandbox is not a compatibility path.
